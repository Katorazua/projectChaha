...
<div class="wrapper">
    <section class="chat-area">
        <header>
            ...
            ?>
            <a href="../doc/view-appointment.php" class="back-icon"><i class="bi bi-arrow-left-circle-fill"></i></a>
                        ...
                        <div class="chat outgoing">
		 				...
		 				<?php 
                            ...
                            <div class="chat incoming">
                                <?php
                                // fetch author from doctors table  using author_id
                                $user_id = $incoming['user_id'];
                                $query = "SELECT avatar FROM users WHERE id = $user_id";
                                $result = mysqli_query($connection, $query);
                                $user = mysqli_fetch_assoc($result);
                                ?>
                                <img src="../images/<?= $user['avatar'] ?>" alt="photo">
                                <div class="details">
                                    <p><?= $row['message'] ?>
                                        <br><br>
                                        <?= date("M D, Y - H:i", strtotime($row['date_time'])) ?>
        ...
        <form action="#" method="POST" class="typing-area" autocomplete="off" enctype="multipart/form-data">
            <input type="hidden" name="incoming_id" value="<?= $incoming['user_id'] ?>">
