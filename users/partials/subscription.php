...
if (isset($_SESSION['user-id'])) {
    ...
    $stmt = $connection->prepare("SELECT subscription_start_date FROM users WHERE id = $user_id ");
    $stmt->execute();
...
if ($days_remaining <= 1) {
    $notification_message = "Dear user, your subscription will expire on " . $subscription_expiry_date . ". Please renew your subscription.";
...
if ($days_remaining < 1) {
    ...
    $role = "Bronze";
    // update users data
    $stmt = $connection->prepare("UPDATE users SET status = ?, role = ?");
    $stmt->bind_param("ss", $subscription_end_date, $role);
    $stmt->execute();
}
