...
if (isset($_GET['token'])) {
    ...
    $stmt = $connection->prepare("SELECT * FROM doctors WHERE reset_token_hash = ? AND reset_token_expiration > ?");
    ...
    if ($result->num_rows > 0) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
          ...
          $stmt = $connection->prepare("UPDATE doctors SET password = ?, reset_token_hash = NULL, reset_token_expiration = NULL WHERE reset_token_hash = ?");
