...
include '../config/database.php';
  if (isset($_POST['submit'])) {
    ...
    if (!$firstname){
        ...
        $_SESSION['edit-user'] = "please enter employee's gender";  
    } elseif (!$userfield) {
        $_SESSION['edit-user'] = "please enter employee's userfield";  
    } elseif (!$email) {
        $_SESSION['edit-user'] = "please enter a valied email";
    } elseif (!$phone) {
        $_SESSION['edit-user'] = "please enter employee's phone number";
    } elseif (!$addr) {
        $_SESSION['edit-user'] = "please enter employee's address";
    } elseif (!$city) {
        $_SESSION['edit-user'] = "please enter employee's city/country";
    } else {
    ...
    $user_query = "UPDATE users SET firstname='$firstname', lastname='$lastname', email='$email', username='$username', userfield='$userfield', gender='$gender', phone='$phone', city='$city', addr='$addr', avatar='$avatar_to_insert' WHERE id = $id LIMIT 1";
